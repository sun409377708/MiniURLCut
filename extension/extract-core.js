(function (root) {
  ...
  MiniURLCut.getLeafElements = function (root) {
    ...
    return MiniURLCut.isBlockCandidate(root) ? [root] : [];
  };

  MiniURLCut.getBlocks = function (root) {
    return MiniURLCut.getLeafElements(root).map((el, index) => ({
      index,
  ...
  MiniURLCut.markLiveBlocks = function (doc) {
    const d = doc || document;
    d.querySelectorAll("[data-muc-i]").forEach((el) => el.removeAttribute("data-muc-i"));
    const body = MiniURLCut.findBody(d);
    if (!body) return 0;
    MiniURLCut.getLeafElements(body).forEach((el, index) => {
      el.setAttribute("data-muc-i", String(index));
    });
    return MiniURLCut.getLeafElements(body).length;
  };

  MiniURLCut.highlightRange = function (doc, lo, hi) {
    const d = doc || document;
    d.querySelectorAll("[data-muc-i]").forEach((el) => {
      const i = Number(el.getAttribute("data-muc-i"));
      const on = lo != null && hi != null && i >= lo && i <= hi;
      el.style.outline = on ? "3px solid #c48a12" : "";
